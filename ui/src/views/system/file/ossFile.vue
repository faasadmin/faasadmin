<template>
  <div class="app-container">
    ...
    <el-tabs v-model="activeName"  @tab-click="handleTabClick">
      <el-tab-pane :label="dict.label" :name="dict.value" v-for="dict in ossTypeDictDatas" :key="dict.value"></el-tab-pane>
    </el-tabs>
        ...
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button type="primary" icon="el-icon-plus" size="mini" @click="handleAddFolder"
                       v-hasPermi="['system:file:create']">新增文件夹</el-button>
            <el-button type="primary" icon="el-icon-plus" size="mini" @click="handleUploadFile"
                       v-hasPermi="['system:file:create']">上传文件</el-button>
          </el-col>
        ...
        <el-table v-loading="loading" :data="list">
          ...
          <el-table-column label="链接" align="center" prop="link">
            <template slot-scope="scope">
              <a :href="getFileUrl +  scope.row.filePath">链接</a>
            </template>
          </el-table-column>
          <el-table-column label="文件内容" align="center" prop="content">
            <template slot-scope="scope">
              <img v-if="scope.row.fileType == '2'"
                   width="90px" height="90px" :src="getFileUrl +  scope.row.filePath">
              <i v-else>非图片，无法预览</i>
            </template>
          </el-table-column>
...
import {downloadOssFile} from "@/api/system/ossFile";
...
export default {
  ...
  data() {
    return {
      activeName: '',
      //downloadFileUrl: process.env.VUE_APP_BASE_API + '/api/system/file/record/download/bizId/',
      getFileUrl: this.$store.state.config.configData.ossDomain + '/',
      // 遮罩层
      loading: true,
      // 显示搜索条件
      showSearch: true,
      ...
      upload: {
        ...
        // 上传的地址
        url: process.env.VUE_APP_BASE_API + '/api/' + "/system/file/record/uploadOss",
        data:{
          fid:undefined,
        },
      },
  ...
  created() {
    console.info('链接：' + this.$store.state.config.configData);
    debugger
    if(this.activeName === ''){
      debugger
      this.activeName = this.ossTypeDictDatas[0].value;
      this.queryParams.ossType = this.ossTypeDictDatas[0].value;
    }
    this.getList();
    this.getTreeselect();
    debugger
    console.log(this.$store.state.config.configData);
    console.log(this.$store.state.config.firstFlag);
    console.log(this.$store.getters.firstFlag)
    if(this.$store.getters.firstFlag){
      getDictDatas(DICT_TYPE.SYS_OSS_TYPE).shift()
      this.firstFlag=false;
      this.$store.dispatch('config/setFirstFlag',false)
    }

  },
  methods: {
    handleTabClick(tab, event) {
      this.queryParams.ossType=tab.name;
      this.handleQuery();
    },
    /** 查询文件列表 */
    getList() {
    ...
    handleDownload(row){
      const ids = row.id;
      downloadOssFile(ids).then(response => {
        this.downloadOctet(response, row.fileName);
      });
    },
    /** 删除按钮操作 */
    handleDelete(row) {
