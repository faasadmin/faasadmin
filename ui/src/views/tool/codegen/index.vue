    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          v-hasPermi="['tool:codegen:create']"
        >生成</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['tool:codegen:delete']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
...
import {
  ...
  syncCodegenFromDB, syncCodegenFromSQL, createCodegenListFromSQL, batchDownloadCodegen
...
export default {
  ...
  methods: {
    ...
    handleGenTable(row) {
      ...
      batchDownloadCodegen(tableNames).then(response => {
        this.downloadZip(response, 'codegen-' + tableNames + '.zip');
      })
    },
    /** 同步数据库操作 */
    handleSynchDb(row) {
